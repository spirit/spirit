...
describe('load-animation', () => {
  ...
  describe('parse', () => {
    describe('with animationData', () => {
      it('should create animations from object', async () => {
        ...
        expect(isGSAPTimeline(result)).to.be.true;
      ...
      it('should create animations from multi array', async () => {
        const result = await loadAnimation({
          animationData: [{ name: 'a' }, { name: 'b' }, { name: 'c' }],
        });
      ...
      it('should throw error on invalid animation data', async () => {
        ...
        expect(await load('aa'))
          .to.be.an('error')
          .to.match(/Invalid animation data/);
        expect(await load(1234))
          .to.be.an('error')
          .to.match(/Invalid animation data/);
        expect(await load({}))
          .to.be.an('error')
          .to.match(/Cannot create group without a name/);
      });
    });
    ...
    describe('with load path', () => {
      ...
      it('should create animations from multi array', async () => {
        stubXhr(sandbox, {
          responseText: JSON.stringify([{ name: 'a' }, { name: 'b' }, { name: 'c' }]),
        });
  ...
  describe('features', () => {
    ...
    const createSimpleGroup = (
      ...
      path,
      opts = {},
      props = { x: { '0s': 0, '0.001s': 100 } }
    ) => ({
      name,
      timelines: [{ path, props }],
      ...opts,
    });
    ...
    describe('loop', () => {
      ...
      it('should loop all groups', async () => {
        ...
        expect(Object.keys(groups).map(g => groups[g].repeat())).to.deep.equal([
          -1,
          -1,
          -1,
        ]);
    ...
    describe('yoyo', () => {
      ...
      it('should yoyo all groups', async () => {
        ...
        expect(Object.keys(groups).map(g => groups[g].yoyo())).to.deep.equal([
          true,
          true,
          true,
        ]);
    ...
    describe('delay', () => {
      ...
      it('should delay all groups', async () => {
        ...
        expect(Object.keys(groups).map(g => groups[g].repeatDelay())).to.deep.equal([
          2,
          2,
          2,
        ]);
    ...
    describe('time scale', () => {
      ...
      it('should overwrite time scale for all groups', async () => {
        ...
        expect(Object.keys(groups).map(g => groups[g].timeScale())).to.deep.equal([
          4,
          4,
          4,
        ]);
