...
describe('parser', () => {
  ...
  describe('create', () => {
    ...
    it('should fail to create in node', () => {
      ...
      expect(() => create({}, document.body)).to.throw(
    ...
    describe('element found', () => {
      ...
      it('should resolve element by id', () => {
        ...
        expect(groups.get('ghost-animation').timelines.get(post1)).to.be.an.instanceOf(
          Timeline
        );
      });
      ...
      it('should resolve element by path expression', () => {
        ...
        const timeline = create(group)
          .get('ghost-animation')
          .timelines.at(0);

        expect(timeline).to.have.property(
          'transformObject',
          post2.querySelector('.post-args')
        );
    ...
    describe('with sub container as root element', () => {
      ...
      it('should resolve element by id from sub container', () => {
        ...
        expect(groups).to.have.lengthOf(1);
        expect(groups.get('ghost-animation').timelines.at(0)).to.have.property(
          'transformObject',
          firstPost
        );
        expect(
          groups.get('ghost-animation').timelines.get(firstPost)
      ...
      it('should resolve element by path expression from sub container', () => {
        ...
        expect(
          groups.get('ghost-animation').timelines.get(secondPost)
        ).to.be.an.instanceOf(Timeline);
      });
    });
    ...
    describe('exported root', () => {
      ...
      it('should ignore exported root if root element is provided', () => {
        ...
        expect(g.timelines.at(0)).to.have.property(
          'transformObject',
          container.querySelector('.container-b')
        );
        expect(g.timelines.at(1)).to.have.property(
          'transformObject',
          container.querySelector('.post-a3')
        );
      ...
      it('should use exported root if no root element is provided', () => {
        ...
        expect(g.timelines.at(0)).to.have.property(
          'transformObject',
          container.querySelector('.post-b1')
        );
        expect(g.timelines.at(1)).to.have.property(
          'transformObject',
          container.querySelector('.post-b2')
        );
      ...
      it('should not be able to parse external root and resolve root by document.body', () => {
        ...
        expect(g.timelines.at(0)).to.have.property(
          'transformObject',
          container.querySelector('.container-a')
        );
  ...
  describe('load', () => {
    ...
    it('should fail to create in node', async () => {
      ...
      expect(result)
        .to.be.an('error')
        .to.match(/can only be executed in the browser/);
    });
    ...
    it('should fail to load json', async () => {
      ...
      expect(result)
        .to.be.an('error')
        .to.match(/Unable to load animation\.json/);
    });
    ...
    describe('parse', () => {
      ...
      it('should parse animation and create valid groups', async () => {
        ...
        expect(timelines.at(0))
          .to.have.property('transformObject')
          .to.equal(gc.querySelector('svg'));
        expect(timelines.at(1))
          .to.have.property('transformObject')
          .to.equal(gc.querySelector('path:first-child'));
        expect(timelines.at(2))
          .to.have.property('transformObject')
          .to.equal(gc.querySelector('g'));
        expect(timelines.at(3))
          .to.have.property('transformObject')
          .to.equal(gc.querySelector('path:last-child'));
      });
    });
  });
});
