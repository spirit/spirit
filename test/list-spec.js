...
describe('list', () => {
  ...
  describe('parse on creation', () => {
    ...
    it('should fail when model could not be parsed', () => {
      ...
      expect(() => new List([{ foo: 'bar' }, { bar: 'foo' }], Model)).to.throw(
        /Could not parse/
      );
    });
  });
  ...
  describe('#add', () => {
    ...
    describe('array with model', () => {
      ...
      it('should add by array model instances', () => {
        list.add([new Model({ x: 10 }), new Model({ y: 100 }), new Model({ z: 1000 })]);
      ...
      it('should return added value', () => {
        expect(list.add({ a: 'b' }))
          ...
          .and.have.property('a', 'b');

        const added = list.add([{ b: 'c' }, { c: 'd' }]);
        expect(added).to.be.an('array');
        expect(added[0])
          .to.be.an.instanceOf(Model)
          .and.have.property('b', 'c');
        expect(added[1])
          .to.be.an.instanceOf(Model)
          .and.have.property('c', 'd');
      });
  ...
  describe('#remove', () => {
    ...
    describe('with model', () => {
      ...
      beforeEach(() => {
        list = new List([{ a: 'b' }, { b: 'c' }, { c: 'd' }, { d: 'e' }], Model);
      });
      ...
      it('should remove model by instance', () => {
        ...
        expect(list.list.map(i => i.obj)).to.deep.equal([{ b: 'c' }, { d: 'e' }]);
      });

      it('should remove array of instances', () => {
        list.remove([list.at(0), list.at(2), list.at(3)]);
        expect(list.list.map(i => i.obj)).to.deep.equal([{ b: 'c' }]);
        expect(list).to.have.lengthOf(1);
      });

      it('should return removed value', () => {
        expect(list.remove(list.at(0)))
          .to.be.an.instanceof(Model)
          .to.have.nested.property('obj.a', 'b');

        const removed = list.remove([list.at(0), list.at(2)]);
        expect(removed).to.be.an('array');
        expect(removed[0])
          .to.be.an.instanceOf(Model)
          .to.have.nested.property('obj.b', 'c');
        expect(removed[1])
          .to.be.an.instanceOf(Model)
          .to.have.nested.property('obj.d', 'e');
      });
    });
  });
  ...
  describe('#each', () => {
    ...
    it('should return mapped data', () => {
      expect(
        new List([{ a: 1 }, { a: 2 }, { a: 3 }, { a: 4 }]).each((item, i) => ({
          b: item.a * i,
        }))
      ).to.deep.equal([{ b: 0 }, { b: 2 }, { b: 6 }, { b: 12 }]);
    });
  ...
  describe('#toObject', () => {
    it('should convert to readable object from primitives', () => {
      ...
      expect(new List([{ a: 'a' }, { b: 'b' }]).toArray()).to.deep.equal([
        { a: 'a' },
        { b: 'b' },
      ]);
    });
    ...
    it('should convert to readable object from model instances', () => {
      ...
      const list = new List([{ a: 'a' }, { b: 'b' }, { c: 'c' }], Model);
  ...
  describe('duplicates', () => {
    ...
    it('should fail with dups on property level', () => {
      const list = new List([{ prop: 'x' }, { prop: 'x' }, { prop: 'z' }]);
      expect(() => (list.duplicates = { prop: 'prop' })).to.throw(/List has duplicates/);
    });

    it('should continue with no dups on property level', () => {
      const list = new List([{ prop: 'x' }, { prop: 'y' }, { prop: 'z' }]);
      expect(() => (list.duplicates = { prop: 'prop' })).not.to.throw(
        /List has duplicates/
      );
    });
  ...
  describe('sort on', () => {
    ...
    it('should sort on custom function', () => {
      ...
      list.sortOn = (a, b) =>
        parseFloat(Object.keys(a)[0]) - parseFloat(Object.keys(b)[0]);
  ...
  describe('linked list', () => {
    ...
    beforeEach(() => {
      list = new List([{ frame: 50 }, { frame: 33 }, { frame: 60 }, { frame: 2 }]);
    ...
    const assertLinkedListAscent = () => {
      for (let i = 0; i < list.length; i++) {
        expect(list.at(i)).to.have.property(
          '_next',
          i === list.length - 1 ? null : list.at(i + 1)
        );
      }
    };
    ...
    it('should fail to set linked list on primitives', () => {
      ...
      expect(() => (list.list = ['a', 'b', 'c', 'd'])).to.throw(
  ...
  describe('dispatch changes', () => {
    ...
    describe('add', () => {
      ...
      it('should listen for add events for objects with model', () => {
        ...
        expect(spy.getCall(1).args[0])
          .to.be.an.instanceOf(Model)
          .and.have.property('b', 'c');
        expect(spy.getCall(2).args[0])
          .to.be.an.instanceOf(Model)
          .and.have.property('c', 'd');
        expect(spy.getCall(3).args[0])
          .to.be.an.instanceOf(Model)
          .and.have.property('d', 'e');
      });
    });
    ...
    describe('remove', () => {
      ...
      it('should listen for remove events for objects with model', () => {
        const list = new List([{ a: 'b' }, { b: 'c' }, { c: 'd' }, { d: 'e' }], Model);
