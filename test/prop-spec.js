...
describe('prop', () => {
  it('should create an empty property', () => {
    ...
    expect(prop)
      .to.have.property('keyframes')
      .to.be.an.instanceOf(Keyframes)
      .to.have.lengthOf(0);
  });
  ...
  describe('from object', () => {
    ...
    it('should fail on parse invalid time', () => {
      expect(() => Prop.fromObject({ x: { 'invalid-time': { value: 10 } } })).to.throw(
        /Invalid time/
      );
    });

    it('should fail on parse invalid value', () => {
      expect(() => Prop.fromObject({ x: { '1s': { novalue: 123 } } })).to.throw(
        /No value found/
      );
    });
  ...
  describe('dispatch events', () => {
    ...
    it('should dispatch keyframe add', () => {
      ...
      expect(spyA.getCall(0).args[0].toObject()).to.deep.equal({
        '1s': { value: 0, ease: null },
      });
      expect(spyA.getCall(1).args[0].toObject()).to.deep.equal({
        '2s': { value: 100, ease: null },
      });
      expect(spyA.getCall(2).args[0].toObject()).to.deep.equal({
    ...
    it('should dispatch keyframe change list', () => {
      ...
      prop.keyframes.list = [new Keyframe(0, 0), new Keyframe(1, 10)];
