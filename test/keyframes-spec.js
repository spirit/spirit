...
describe('keyframes', () => {
  ...
  describe('#at', () => {
    it('should fail on retrieve invalid index', () => {
      expect(() => new Keyframes({ '10s': { value: 0 } }).at(2)).to.throw(
        /Index exceeded/
      );
    });
  ...
  describe('#add', () => {
    ...
    it('should add by object(s)', () => {
      ...
      expect(keyframes.at(0).toObject()).to.deep.equal({
        '2s': { value: 200, ease: null },
      });
      expect(keyframes.at(1).toObject()).to.deep.equal({
        '3s': { value: 1000, ease: null },
      });
      expect(keyframes.at(2).toObject()).to.deep.equal({
        '5s': { value: 300, ease: null },
      });
      expect(keyframes.at(3).toObject()).to.deep.equal({
        '10s': { value: 400, ease: null },
      });
    });
    ...
    it('should add by array instances', () => {
      ...
      expect(keyframes.at(0).toObject()).to.deep.equal({
        '2s': { value: 300, ease: 'Power3.easeInOut' },
      });
      expect(keyframes.at(1).toObject()).to.deep.equal({
        '10s': { value: 100, ease: 'Linear.easeNone' },
      });
    });
  });
  ...
  describe('duplicates', () => {
    it('should not allow time duplicates', () => {
      ...
      expect(() => keyframes.add({ '0.2s': { value: 200 } })).to.throw(
        /List has duplicates/
      );
      expect(
        () => new Keyframes([{ '0.2s': { value: 1000 } }, { '0.2s': { value: 50 } }])
  ...
  describe('mappings', () => {
    let mappings, keyframes;

    beforeEach(() => {
      mappings = [new EvalMap(/foo/, 'this is foo!'), new EvalMap(/bar/, 'this is bar!')];
    ...
    it('should evaluate mapping', () => {
      ...
      keyframes.mappings = [...mappings, new EvalMap(/hello/, name => `hello ${name}`)];

      expect(keyframes.add(new Keyframe(50, '{hello("there")}'))).to.have.property(
        'value',
        'hello there'
      );
    });
