...
describe('props', () => {
  ...
  describe('duplicates', () => {
    it('should not allow name duplicates', () => {
      ...
      expect(() => new Props([{ x: {} }, { x: {} }])).to.throw(/List has duplicates/);
    });
  });
  ...
  describe('linked list', () => {
    ...
    beforeEach(() => {
      properties = new Props([new Prop('z'), new Prop('x'), new Prop('y')]);
    });
  ...
  describe('#get', () => {
    ...
    it('should get property', () => {
      ...
      expect(properties.get('z')).to.have.nested.property(
        'keyframes.list[0].value',
        1000
      );
    });
  ...
  describe('#add', () => {
    ...
    it('should add by array object', () => {
      properties.add([{ x: { '0s': { value: 10 } } }, { y: { '0s': { value: 0 } } }]);
  ...
  describe('#toArray', () => {
    it('should convert a valid array', () => {
      ...
      expect(properties.toArray()).to.deep.equal([{ x: val }, { y: val }, { z: val }]);
    });
  });
  ...
  describe('dispatch changes', () => {
    let keyframe, keyframes, prop, properties, spy;
    ...
    beforeEach(() => {
      ...
      keyframes = new Keyframes([keyframe, new Keyframe(1, 100), new Keyframe(2, 1000)]);
    ...
    it('should change property keyframes', () => {
      ...
      const expectSpy = spy => {
        ...
        expect(spy.getCall(0).args[0])
          .to.have.nested.property('changed.from')
          .to.deep.equal([
            { '0s': { value: 0, ease: null } },
            { '1s': { value: 100, ease: null } },
            { '2s': { value: 1000, ease: null } },
        ...
        expect(spy.getCall(0).args[0])
          .to.have.nested.property('changed.to')
          .to.deep.equal({
            '0s': { value: 0, ease: null },
            '1s': { value: 0, ease: null },
          });
    ...
    it('should change keyframe ease', () => {
      ...
      const expectSpy = spy => {
        ...
        expect(spy.getCall(0).args[0])
          .to.have.nested.property('changed.to')
          .to.equal('Power3.easeOut');
        expect(spy.getCall(1).args[0])
          .to.have.nested.property('changed.from')
          .to.equal(null);
        expect(spy.getCall(1).args[0])
          .to.have.nested.property('changed.to')
          .to.equal('Strong.easeInOut');
      };
    ...
    it('should add keyframe', () => {
      ...
      expect(spy.getCall(0).args[0].toObject()).to.deep.equal({
        '10s': { value: 10.5, ease: null },
      });
      expect(spy.getCall(1).args[0]).to.be.an.instanceOf(Keyframe);
      expect(spy.getCall(1).args[0].toObject()).to.deep.equal({
        '12s': { value: 10, ease: null },
      });
    });
