...
describe('keyframe', () => {
  ...
  describe('eval mapping', () => {
    ...
    it('should evaluate multiple mappings', () => {
      ...
      keyframe.mappings = [
        new EvalMap(/one/, function() {
          return 1;
        }),
        new EvalMap(/two/, function() {
          return 2;
        }),
      ];
  ...
  describe('from object', () => {
    ...
    it('should fail on parse invalid time', () => {
      expect(() => Keyframe.fromObject({ 'invalid-time': { value: 10 } })).to.throw(
        /Object is invalid/
      );
    });

    it('should fail on parse invalid value', () => {
      expect(() => Keyframe.fromObject({ '1s': { value: undefined } })).to.throw(
        /Object is invalid/
      );
      expect(() => Keyframe.fromObject({ '1s': { value: null } })).to.throw(
        /Object is invalid/
      );
    });
  ...
  describe('to object', () => {
    it('should convert to object ignoring eval', () => {
      ...
      expect(keyframe.toObject()).to.deep.equal({
        '0s': { value: 133, ease: 'Expo.easeOut' },
      });
      expect(keyframe.toObject(true)).to.deep.equal({
        '0s': { value: '{ test + 10 }', ease: 'Expo.easeOut' },
      });
    });
  });
